def Add_FullNC_Peak_Profiles(metabolite_dict,metabolite_list):
    ...
    FullNC_Metabolites = ['alanine_2tbdms','glycine_2tbdms','valine_2tbdms','leucine_2tbdms','isoleucine_2tbdms','serine_3tbdms', \
                          ...
                          'histidine_3tbdms','tryptophan_3tbdms','glucose_clean','glucose_clean2']

    # Adjust the peak profiles to search for labeled internal standard because the unlabeled metabolite may not be present
    for metabolite in metabolite_list:
        if metabolite in FullNC_Metabolites:
            ...
            for key in Fragment_Keys:
                ...
                # Only integrate the unlabeled parent M0 and the parent of the labeled internal standard
                unlabaled_parent = Fragment_Parent_MZ
                labeled_parent = Fragment_Parent_MZ + Adjustment_to_Peak_Profile
                metabolite_dict[metabolite]['fragments'][key]['mzs_to_integrate'] = np.array([unlabaled_parent,labeled_parent])

    # For metabolites without a labeled internal standard, only integrate M0
    for metabolite in metabolite_list:
        Fragment_Keys = metabolite_dict[metabolite]['fragments'].keys()
        for key in Fragment_Keys:
            if (key=='lactate_233') | (key=='lactate_261') | (key=='HPA_261'):
                unlabaled_parent = metabolite_dict[metabolite]['fragments'][key]['mzs_to_integrate'][0]
                metabolite_dict[metabolite]['fragments'][key]['mzs_to_integrate'] = np.array([unlabaled_parent])
